...
(deftype UByteType []
  ...
  (write [bt buffer idx value]
    (.setByte ^io.netty.buffer.UnpooledHeapByteBuf buffer idx value))
  (read [by buffer idx]
    (bit-and 0xFF (.getByte ^io.netty.buffer.UnpooledHeapByteBuf buffer idx))))
...
(defn fill-buffer [buf b]
  (.setBytes ^io.netty.buffer.UnpooledHeapByteBuf (bc/buffer buf) 0 ^bytes b)
  buf)
...
(defn get-bytes
  ...
  ([buf start length]
   (let [b (byte-array length)]
     ;; hmm, these hints seem off
     (.getBytes ^io.netty.buffer.UnpooledHeapByteBuf buf ^long start ^bytes b 0 ^long length)
     b)))

(defn encode-token
  [{:keys [version ^bytes ciphertext iv timestamp hmac-fn]}]
  (let [ciphertext-length (alength ciphertext)
    ...
    (.array ^io.netty.buffer.UnpooledHeapByteBuf (bc/buffer buf))))
